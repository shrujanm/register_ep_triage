...
import sys
sys.path.insert(0, "register_ep_triage/src")
...
class IntrospectionTokenVerifier(TokenVerifier):
    ...
    def __init__(
        ...
        print(f"IntrospectionTokenVerifier initialized with endpoint: {self.introspection_endpoint}"
              f", server_url: {self.server_url}, resource_url: {self.resource_url},"
              f" validate_resource: {self.validate_resource}")
    ...
    async def verify_token(self, token: str) -> AccessToken | None:
        ...
        async with httpx.AsyncClient(
            ...
            try:
                logger.info(f"Sending introspection request to {self.introspection_endpoint}")
                response = await client.post(
                    self.introspection_endpoint,
                    data={"token": token},
                    headers={"Content-Type": "application/x-www-form-urlencoded"},
                )
                logger.info(f"Received introspection response: status={response.status_code}")
                ...
                if not data.get("active", False):
                    logger.info("Token is not active according to introspection response")
                    return None
    ...
    def _validate_resource(self, token_data: dict[str, Any]) -> bool:
        ...
        if not self.server_url or not self.resource_url:
            logger.warning("Resource validation failed: server_url or resource_url is missing")
    ...
    def _is_valid_resource(self, resource: str) -> bool:
        ...
        if not self.resource_url:
            logger.info("Resource validation failed: resource_url is not configured")
            return False
        
        logger.info(f"Validating resource: requested={self.resource_url}, configured={resource}")
