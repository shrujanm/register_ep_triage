...
class SimpleOAuthProvider(OAuthAuthorizationServerProvider[AuthorizationCode, RefreshToken, AccessToken]):
    ...
    async def get_client(self, client_id: str) -> OAuthClientInformationFull | None:
        ...
        if not client:
            logger.warning(f"Client ID {client_id} not found. Temporarily creating a public client.")
            # For demo purposes, create a public client on the fly
            ...
            logger.info(f"Registered temporary public client: {client_info}")
    ...
    async def authorize(self, client: OAuthClientInformationFull, params: AuthorizationParams) -> str:
        ...
        print(f"Authorize called with client: {client}, params: {params}, state: {state}")
    ...
    async def get_login_page(self, state: str) -> HTMLResponse:
        ...
        <body>
            ...
            <form action="{self.server_url.rstrip("/")}/login/callback" method="post">
                <input type="hidden" name="state" value="{state.replace("+", "%2B")}">
    ...
    async def handle_login_callback(self, request: Request) -> Response:
        ...
        # URL decode the state to handle + characters correctly
    ...
    async def handle_simple_callback(self, username: str, password: str, state: str) -> str:
        ...
        print(f"Handling callback for user: {username}, state: {state}")
        print(f"Available states: {self.state_mapping.keys()}")
